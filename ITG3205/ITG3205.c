...
float g_offx = 0; /* Смещение по x */
float g_offy = 0; /* Смещение по y */
float g_offz = 0; /* Смещение по z */

/* Буфера данных (менять не нужно) *********************/
byte buff[G_TO_READ]; /* Буфер сырых данных гироскопа */
float GyroCal[3];		  /* Буфер для калибровки *********/
float rawDataGyro[3];
// Калибровка ************************************************************************************************/
void GyroCalibrate() {
...
void getGyroscopeData(float *result) {
	...
	**************************************/
	...
	result[0] = rawDataGyro[0] / 14.375  - g_offx; /* Скорость вращения по x*/
	result[1] = rawDataGyro[1] / 14.375  - g_offy; /* Скорость вращения по y*/
	result[2] = rawDataGyro[2] / 14.375  - g_offz; /* Скорость вращения по z*/
}
/**********************************************/
